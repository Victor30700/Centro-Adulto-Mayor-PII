...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/Orientacion/RegistrarFicha.css') }}">
...
<div class="container1">
    <div class="navigation-buttons1">
        <a href="{{ route('admin.orientacion.index') }}" class="btn">← Volver al listado</a>
    </div>
    ...
    <div class="form-section">
        <form id="ficha-orientacion-form" action="{{ $modoEdicion ? route('admin.orientacion.update', ['cod_or' => optional($orientacion)->cod_or]) : route('admin.orientacion.store') }}" method="POST">
            @csrf
            @if($modoEdicion)
                @method('PUT')
            @endif
