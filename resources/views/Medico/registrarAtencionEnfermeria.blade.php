...
<div class="container1">
    <div class="navigation-buttons">
        <a href="{{ route('admin.enfermeria.index') }}" class="btn btn-secondary">← Volver al listado</a>
    </div>
    ...
    <div class="form-section">
        <form action="{{ $modoEdicion ? route('admin.enfermeria.update', ['cod_enf' => optional($fichaEnfermeria)->cod_enf]) : route('admin.enfermeria.store', ['id_adulto' => $adulto->id_adulto]) }}" method="POST">
            @csrf
            @if($modoEdicion)
                @method('PUT')
            @endif
