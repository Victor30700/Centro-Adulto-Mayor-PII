...
class RegistrarCasoController extends Controller
{
    ...
    public function showDetalle($id_adulto)
    {
        try {
            ...
            return redirect()->route('admin.reportes_proteccion.index')->with('error', 'El caso que intentas ver no existe o ha sido eliminado.');
        } catch (\Exception $e) {
    ...
    public function storeActividad(Request $request, $id_adulto)
    {
        ...
        if ($request->has('_skip_actividad_laboral') && $request->input('_skip_actividad_laboral') === '1') {
            ...
            try {
                ...
                return redirect()->route('admin.caso.edit', ['id_adulto' => $id_adulto, 'active_tab' => 'encargado'])
    ...
    public function storeEncargado(Request $request, $id_adulto)
    {
        ...
        if ($request->tipo_encargado === 'natural') {
            $rules = array_merge($rules, [
                ...
                'encargado_natural.ci'                => 'required|string|max:20',
        ...
        try {
            ...
            // Validar duplicidad de CI para Persona Natural
            if ($request->tipo_encargado === 'natural') {
                $ci = $request->input('encargado_natural.ci');
                // Verificar si el CI ya existe en Anexo N3 para este id_adulto
                $anexoN3Exists = AnexoN3::where('id_adulto', $id_adulto)
                    ->whereExists(function ($query) use ($ci) {
                        $query->select(DB::raw(1))
                              ->from('persona_natural')
                              ->whereRaw('persona_natural.id_natural = anexo_n3.id_natural')
                              ->where('persona_natural.ci', $ci);
                    })
                    ->exists();

                if ($anexoN3Exists) {
                    throw ValidationException::withMessages([
                        'encargado_natural.ci' => ['El CI ya está registrado en Anexo N3 para este caso.']
                    ]);
                }
            }
            ...
            if ($request->tipo_encargado === 'natural') {
                ...
                $personaNatural->fill($request->input('encargado_natural'));
                $personaNatural->id_encargado = $encargado->id_encargado;
                $personaNatural->save();
            } else {
            ...
            return redirect()->route('admin.caso.edit', ['id_adulto' => $id_adulto, 'active_tab' => 'denunciado'])
    ...
    public function storeDenunciado(Request $request, $id_adulto)
    {
        ...
        try {
            ...
            return redirect()->route('admin.caso.edit', ['id_adulto' => $id_adulto, 'active_tab' => 'grupo'])
    ...
    public function storeGrupoFamiliar(Request $request, $id_adulto)
    {
        ...
        try {
            ...
            return redirect()->route('admin.caso.edit', ['id_adulto' => $id_adulto, 'active_tab' => 'croquis'])
    ...
    public function storeCroquis(Request $request, $id_adulto)
    {
        ...
        try {
            ...
            return redirect()->route('admin.caso.edit', ['id_adulto' => $id_adulto, 'active_tab' => 'seguimiento'])
    ...
    public function storeSeguimiento(Request $request, $id_adulto)
    {
        ...
        try {
            ...
            return redirect()->route('admin.caso.edit', ['id_adulto' => $id_adulto, 'active_tab' => 'intervencion'])
    ...
    public function storeIntervencion(Request $request, $id_adulto)
    {
        ...
        try {
            ...
            return redirect()->route('admin.caso.edit', ['id_adulto' => $id_adulto, 'active_tab' => 'anexo3'])
    ...
    public function storeAnexoN3(Request $request, $id_adulto)
    {
        ...
        try {
            ...
            return redirect()->route('admin.caso.edit', ['id_adulto' => $id_adulto, 'active_tab' => 'anexo5'])
    ...
    public function storeAnexoN5(Request $request, $id_adulto)
    {
        ...
        try {
            ...
            return redirect()->route('admin.caso.index')->with('success', 'Datos del caso (incluyendo Anexo N5) registrados exitosamente.');
    ...
    public function destroy($id_adulto)
    {
        ...
        try {
            ...
            return redirect()->route('admin.reportes_proteccion.index')->with('success', 'Módulo de protección del caso eliminado exitosamente. El registro del Adulto Mayor y la Persona han sido conservados.');

        } catch (ModelNotFoundException $e) {
            DB::rollBack();
            Log::error("Adulto Mayor no encontrado para eliminar módulo de protección con ID: {$id_adulto}. Error: " . $e->getMessage());
            return redirect()->route('admin.reportes_proteccion.index')->with('error', 'El caso que intentas eliminar no existe.');
        } catch (\Exception $e) {
            DB::rollBack();
