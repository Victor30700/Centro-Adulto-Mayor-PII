...
/**
    ...
    public function destroy($cod_or)
    {
        try {
            ...
            $orientacion->delete();
            return redirect()->route('admin.reportes_orientacion.index')->with('success', 'Ficha de orientación eliminada exitosamente.');
        } catch (\Exception $e) {
            Log::error('Error al eliminar ficha de orientación: ' . $e->getMessage());
            return redirect()->route('admin.reportes_orientacion.index')->with('error', 'Error al eliminar la ficha de orientación. Por favor, intente de nuevo.');
        }
    }
    /**
