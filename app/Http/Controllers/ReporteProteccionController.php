...
class ReporteProteccionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Aplicar filtro por número de caso
        ...
        if (!empty($search)) {
            $query->where(function ($q) use ($search) {
                $q->where('nro_caso', 'like', '%' . $search . '%')
                  ->orWhereHas('persona', function ($qr) use ($search) {
        ...
        // Aplicar buscador por nombres y apellidos del Denunciado
        if (!empty($denunciado_search)) {
            $query->whereHas('denunciado.personaNatural', function ($qr) use ($denunciado_search) {
                $qr->where('nombres', 'like', '%' . $denunciado_search . '%')
                   ->orWhere('primer_apellido', 'like', '%' . $denunciado_search . '%')
                   ->orWhere('segundo_apellido', 'like', '%' . $denunciado_search . '%');
            });
        }

        // === FILTRO CLAVE: Asegurar que el Adulto Mayor tenga al menos UN módulo de protección activo ===
        // Esto filtrará los AdultoMayor que ya no tienen ningún registro en las tablas de sus módulos,
        // asumiendo que la ausencia de todos estos módulos significa que el "caso" ya no existe.
        $query->where(function ($q) {
            $q->whereHas('actividadLaboral')   // Verifica si tiene ActividadLaboral asociada
              ->orWhereHas('encargados')       // Verifica si tiene Encargado asociado
              ->orWhereHas('denunciado')       // Verifica si tiene Denunciado asociado
              ->orWhereHas('grupoFamiliar')    // Verifica si tiene GrupoFamiliar asociado
              ->orWhereHas('croquis')          // Verifica si tiene Croquis asociado
              ->orWhereHas('seguimientos')     // Verifica si tiene Seguimientos asociados
              ->orWhereHas('anexoN3')          // Verifica si tiene AnexoN3 asociado
              ->orWhereHas('anexoN5');         // Verifica si tiene AnexoN5 asociado
        });
        // Si un "caso de protección" solo existe si tiene *todos* estos módulos,
        // cambiarías 'orWhereHas' por 'whereHas' y encadenarías las condiciones.
        // La lógica actual ('orWhereHas') significa "si tiene al menos uno de ellos".
    ...
    public function exportarFichaProteccionWordIndividual(int $id_adulto)
    {
        try {
            ...
            if ($persona) {
                ...
                $section->addText('Estado Civil: ' . (optional($adulto)->estado_civil ?? 'N/A'), 'valueStyle', 'P_Start'); 
                $section->addText('Domicilio: ' . (optional($persona)->direccion ?? 'N/A'), 'valueStyle', 'P_Start');
                ...
                $section->addText('De donde?: ' . (optional($adulto)->migrante_de_donde ?? 'N/A'), 'valueStyle', 'P_Start'); 
            } else {
